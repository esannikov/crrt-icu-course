...
- Use the clinical data strip and vertical decision ladder from ideation direction 2 with the bold humanist sans-serif typography from direction 3.
- Use a warm minimal palette: parchment, ink black, deep olive, burnt sienna, pale amber, and muted sage.
- The course is for personal study by an adult anesthesiology intern. It has eight modules with ten scored cards per module, immediate feedback, local progress, and a final result.
- Start with five synthetic adult ICU patients. The selected patient becomes the longitudinal case in the final module; changing the patient preserves the seven common modules.
- Label clinical sources and frame the product as education, not a local treatment protocol or device instruction.
